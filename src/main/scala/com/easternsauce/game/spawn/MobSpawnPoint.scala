  ...
  def update(): Unit = {
    if (isToBeRespawned) {
      ...
      if (creatureType == "skeletonSword") {
        spawnedCreature = new Skeleton(
          "skellie" + Math.abs(GameSystem.random.nextInt),
          this,
          "woodenSword"
        )
      ...
      if (creatureType == "wolf") {
        spawnedCreature =
          new Wolf("wolfie" + Math.abs(GameSystem.random.nextInt), this)
        area.moveInCreature(spawnedCreature, posX, posY)
      }
      if (creatureType == "ghost") {
        spawnedCreature = new Ghost(
          "ghost" + Math.abs(GameSystem.random.nextInt),
          this,
          "woodenSword"
        )
        area.moveInCreature(spawnedCreature, posX, posY)
      ...
      if (creatureType == "goblin") {
        spawnedCreature = new Goblin(
          "goblin" + Math.abs(GameSystem.random.nextInt),
          this,
          "poisonDagger"
        )
        area.moveInCreature(spawnedCreature, posX, posY)
      ...
      if (creatureType == "fireDemon") {
        spawnedCreature = new FireDemon(
          "firedemon" + Math.abs(GameSystem.random.nextInt),
          this,
          "demonTrident"
        )
        area.moveInCreature(spawnedCreature, posX, posY)
