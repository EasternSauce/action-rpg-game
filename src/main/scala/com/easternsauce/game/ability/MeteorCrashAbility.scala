...
class MeteorCrashAbility(override val abilityCreature: Creature) extends Ability(abilityCreature) {
  ...
  override protected def onActiveStart(): Unit = {
    ...
    for (i <- 0 until 10) {
      meteors += Meteor(this, 0.1f * i, abilityCreature.posX + (100 * (i + 1)) * facingVector.x,
        abilityCreature.posY + (100 * (i + 1)) * facingVector.y, 50 + 3 * i * i, 2.5f)
    }
    for (i <- 0 until 10) {
      val vector: Vector2 = facingVector.cpy()
      vector.setAngleDeg(vector.angleDeg() + 50)
      meteors += Meteor(this, 0.1f * i, abilityCreature.posX + (100 * (i + 1)) * vector.x,
        abilityCreature.posY + (100 * (i + 1)) * vector.y, 50 + 3 * i * i, 2.5f)
    }
    for (i <- 0 until 10) {
      val vector: Vector2 = facingVector.cpy()
      vector.setAngleDeg(vector.angleDeg() - 50)
