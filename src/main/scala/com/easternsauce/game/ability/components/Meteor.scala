...
class Meteor private (override val mainAbility: Ability, val startTime: Float, val posX: Float, val posY: Float,
                      val radius: Float, speed: Float) extends AbilityComponent(mainAbility) {

  override protected val activeTime: Float = 1.8f / speed
  override protected val channelTime: Float = 1.2f / speed
  override protected val abilityAnimation: EsAnimation = new EsAnimation(Assets.explosionSpriteSheet,
    0.092f / speed)
  override protected val abilityWindupAnimation: EsAnimation = new EsAnimation(Assets.explosionWindupSpriteSheet,
    0.2f / speed)
  ...
  override def render(shapeDrawer: ShapeDrawer, batch: SpriteBatch): Unit = {
    if (state == AbilityState.Channeling) {
      ...
      batch.draw(image, posX - radius, posY - radius, 0, 0,
