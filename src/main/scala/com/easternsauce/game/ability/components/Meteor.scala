...
import com.easternsauce.game.ability.{Ability, MeteorRainAbility}
...
class Meteor(override val mainAbility: Ability, val startTime: Float, val posX: Float, val posY: Float, val radius: Float, speed: Float) extends AbilityComponent(mainAbility) {
  ...
  override def onCollideWithCreature(creature: Creature): Unit = {
    ...
    if (!(mainAbility.abilityCreature.isMob && creature.isMob) && creature.isAlive) { // mob can't hurt a mob?
      if (!creature.isImmune) creature.takeDamage(40f, immunityFrames = true, 0, 0, 0)
    }
  }
